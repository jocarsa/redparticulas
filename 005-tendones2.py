...
while True:
    ...
    # Create an empty transparent layer for tendons
    tendon_layer = np.zeros((height, width, 4), dtype=np.uint8)  # 4 channels for RGBA
    ...
    for i in range(number_of_particles):
        ...
        for j in range(i + 1, number_of_particles):
            ...
            if distance < min_distance:
                ...
                # Draw the filled polygon (tendon) on the transparent layer
                cv2.fillPoly(tendon_layer, [pts], (0, 0, 0, 255))

    # Apply Gaussian blur to tendon layer for antialiasing
    blurred_tendon_layer = cv2.GaussianBlur(tendon_layer, (0, 0), sigmaX=1.5, sigmaY=1.5)

    # Convert frame to BGRA for blending
    frame_bgra = cv2.cvtColor(frame, cv2.COLOR_BGR2BGRA)

    # Blend the tendon layer onto the frame
    combined = cv2.addWeighted(frame_bgra, 1.0, blurred_tendon_layer, 1.0, 0)

    # Convert back to BGR
    final_frame = cv2.cvtColor(combined, cv2.COLOR_BGRA2BGR)

    # Draw particles on the final frame
    for particle in particles:
        cv2.circle(
            final_frame,
    ...
    # Display the frame
    cv2.imshow('Particle Network Animation', final_frame)
